            ...
            for (int i = 0; i < ready_num; ++i) {
                ...
                // 收到 TLV 包
                    ...
                    //用户命令
                        ...
                        handle_command(&ctx.session[ctx.hashmap[fd]], tlv);
                        tlv_free(tlv);
                        printf("[命令处理] 完成 (FD=%d)\n", fd);
                    }
                }
    ...
    int session_insert(server_context_t* ctx, MYSQL* conn, const char* user_name, int netfd) {
    // 打印输入参数信息
    printf("[测试] 开始 session_insert 函数\n");
    printf("[测试] 参数信息:\n");
    printf("  ctx 地址: %p\n", ctx);
    printf("  MySQL 连接: %p\n", conn);
    printf("  用户名: %s\n", user_name);
    ...
    */
    ...
    // 根据虚拟路径和用户名获取文件id
    int vf_get_id_by_path_user(MYSQL* conn, const char *path, const char *user_name) {
            printf("[DEBUG] 进入 vf_get_id_by_path_user 函数\n");
            printf("[参数] path = '%s', user_name = '%s'\n", path, user_name);
            
            char query[512];
            snprintf(query, sizeof(query),
                    "SELECT id FROM virtual_file WHERE path='%s' AND user_name='%s'", 
                    path, user_name);
    ...
    int usr_register(server_context_t* ctx, tlv_t *tlv, int net_fd, MYSQL* conn)
    {
        ...
        char *crptpswd=(char*)calloc(512,sizeof(char));
        char *salt=(char*)calloc(SALT_LEN,sizeof(char));
        ...
        */
        ...
        free(usr_name);
        free(crptpswd);
        free(salt);    // 注意：这里 salt 的声明有误，应该是 char *salt
        free(token);
    }
